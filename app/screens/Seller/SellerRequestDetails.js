...
export default function SellerRequest() {
    ...
    const restaurantId = 'restaurant_user_2'; // Hardcoded for now, can be dynamic based on user context
    ...
    return (
        <SafeAreaView className="flex-1 bg-gray-100 pt-5">
            ...
            <ScrollView className="flex-1 p-4">
                ...
                {!isFromHistory && (
                    <TouchableOpacity
                        className="bg-[#00CCBB] px-4 py-3 rounded-md mb-4 flex-row justify-center items-center"
                        onPress={() => {
                            navigation.navigate('SellerDonationForm', {
                                restaurantId: 'restaurant_user_2', // Replace after authentication
                                request: request
                            });
                        }}
                    >
                ...
                {!isFromHistory && (
                    <View className="flex-row justify-between mb-10">
                        ...
                        <TouchableOpacity
                            className={`px-4 py-3 rounded-md flex-1 ml-2 flex-row justify-center items-center ${progress.isComplete ? 'bg-green-500/20' : 'bg-yellow-500/20'
                                }`}
                            onPress={markRequestComplete}
                        >
                            <CheckIcon size={18} color={progress.isComplete ? '#10b981' : '#f59e0b'} strokeWidth={2.5} />
                            <Text className={`text-center font-semibold ml-2 ${progress.isComplete ? 'text-green-600' : 'text-yellow-600'
                                }`}>
                                Mark as Complete
                            </Text>
                        </TouchableOpacity>
